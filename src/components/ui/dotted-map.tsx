...
> extends React.SVGProps<SVGSVGElement> {
  ...
  renderUnderlay?: (args: {
    processedMarkers: (MapMarker<M> & { calculatedX: number; calculatedY: number; r: number })[]
    width: number
    height: number
  }) => React.ReactNode
...
export function DottedMap<M extends Marker = Marker>({
  ...
  const calculatedMarkers = React.useMemo(() => {
    return processedMarkers.map((marker) => {
      const rowIndex = yToRowIndex.get(marker.y) ?? 0
      const offsetX = stagger && rowIndex % 2 === 1 ? xStep / 2 : 0
      const calculatedX = marker.x + offsetX
      const calculatedY = marker.y
  ...
  }, [processedMarkers, yToRowIndex, stagger, xStep, dotRadius])
  ...
  return (
    ...
    >
      ...
      {renderUnderlay?.({
        processedMarkers: calculatedMarkers,
        width,
        height,
      })}

      {calculatedMarkers.map((marker, index) => {
        const x = marker.calculatedX
        const y = marker.calculatedY
        const r = marker.r
